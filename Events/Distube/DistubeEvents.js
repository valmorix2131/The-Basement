...
client.distube
  ...
  .on('addSong', (queue, song) =>
    queue.textChannel.send({embeds: [new Message()
    .setColor("GREEN")
  ...
  .on('addList', (queue, playlist) => queue.textChannel.send({embeds: [new MessageEmbed()
    ...
    .setColor("RED")
    .setDescription(`<a:cross:949499323559841872>| An error encountered: ${e.toString().slice(0, 1974)}`)]})
    })
  ...
  .on('searchNoResult', (message, query) => message.channel.send({embeds: [new MessageEmbed()
    .setColor("RED")
    .setDescription(`<a:cross:949499323559841872>| No result found for \`${query}\`!`)]}))
